...
class MinutiaeConnector:
    ...
    async def get_temporal_scaffolding_examples(self) -> List[Dict[str, Any]]:
        ...
        return [
            entry for entry in self.knowledge_index["temporal"]
            if "scaffolding" in entry.get("tags", [])
        ]
    ...
    def _calculate_relevance(self, entry: Dict[str, Any], query_lower: str, 
                           query_tokens: Set[str]) -> float:
    ...
    def _apply_context_boost(self, entry: Dict[str, Any], relevance: float, 
    ...
    def _is_likely_max_object(self, name: str) -> bool:
        ...
        if name.endswith("~"):  # Audio objects
            return True
        if name in ["metro", "counter", "bang", "toggle", "message", "route", 
                    "gate", "sel", "prepend", "append", "pack", "unpack"]:
            return True
        return False
